	...
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback" :bottom="100">
		<view class="container">
				...
				<swiper-item v-for="(swiper,index) in swiperList" :key="index">
					...
					<view class="lable" :style='{"width":"auto","fontSize":"28rpx","color":"rgba(6, 6, 6, 1)","textAlign":"left"}'>科目：</view>
					<view class="text" :style='{"color":"rgba(0, 0, 0, 1)","textAlign":"left","fontSize":"28rpx"}'>{{detail.kemu}}</view>
				</view>
                				                								                								                				<view class="detail-list-item" :style='{"boxShadow":"0 0 0px rgba(0,0,0,.3)","margin":"0 0 2rpx 0","borderColor":"rgba(171, 174, 176, 1)","borderRadius":"0","borderWidth":"2rpx","borderStyle":"none none solid none","height":"68rpx"}'>
				...
				<view class="uni-padding-wrap uni-common-mt">
					<view>
						<video id="myVideo" :src="detail.ziliaoshipin" controls></video>
					</view>
				</view>
			</view>
            			            			            			            			            						
			            			            			            			            			            			            			            			            						
			            			            			            			            			            			                        <view class="time-content" :style='{"boxShadow":"0 0 0px rgba(0,0,0,.3)","margin":"0 0 2rpx 0","borderColor":"#fff","backgroundColor":"#fff","borderRadius":"0","borderWidth":"0","borderStyle":"solid","height":"280rpx"}' style="height:auto !important;">
...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','fuxiziliao');
			...
			onAcrossTap(tableName){
     			uni.setStorageSync('crossTable','fuxiziliao');
				uni.setStorageSync(`crossObj`, this.detail);
				this.$utils.jump(`../${tableName}/add-or-update?cross=true`);
			},
			// 获取详情
			async init(){
				let res = await this.$api.info('fuxiziliao', this.id);
			...
			async upCallback(mescroll) {
				                let res = await this.$api.list('discussfuxiziliao', {
					page: mescroll.num,
					limit: mescroll.size,
					refid: this.id
				});
			...
			// 添加评论
			onCommentTap() {
				this.$utils.jump(`../discussfuxiziliao/add-or-update?refid=${this.id}`)
			},
			// 点赞
			async zan() {
				let _this = this;
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							await _this.$api.update('fuxiziliao', _this.detail);
			...
			async onFinishTap() {
				...
				await this.$api.update('fuxiziliao', this.detail);
				this.$utils.msg('审核成功');
